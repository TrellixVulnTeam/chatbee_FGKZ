<template>
  <div class="page-wrapper">
    <div class="page-section">
      ...
      <div>
        <div class="titlepostuser">
          Post from <nuxt-link :to="'/users/'+user.uuid">{{ user.username }}</nuxt-link>
        </div>
...
export default {
  ...
  methods: {
    ...
    getPosts(pageSelect,status,pageSize) {
      ...
      let apiurl
      if (status == 'normal') {
        apiurl = '/v1/posts.all?page='+pageSelect+'&pageSize=200&uuid='+this.pageslug
      } else {
        apiurl = '/v1/posts.all?page='+pageSelect+'&pageSize='+pageSize+'&'+this.searchTypeSelect+'='+this.search+'&uuid='+this.pageslug
      }
